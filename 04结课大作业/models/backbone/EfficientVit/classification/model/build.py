...
def EfficientViT_M4(num_classes=1000, pretrained=False, distillation=False, fuse=False, pretrained_cfg=None, model_cfg=EfficientViT_m4):
    model = EfficientViT(num_classes=num_classes, distillation=distillation, **model_cfg)
    if pretrained:
        # If a valid file path is provided, load weights from local file;
        # otherwise, fall back to the original URL-based loading.
        if isinstance(pretrained, str) and os.path.isfile(pretrained):
            checkpoint = torch.load(pretrained, map_location='cpu')
        else:
            url = _checkpoint_url_format.format(pretrained)
            checkpoint = torch.hub.load_state_dict_from_url(
                url, map_location='cpu')
        d = checkpoint.get('model', checkpoint)
        D = model.state_dict()
        for k in d.keys():
            if D[k].shape != d[k].shape:
                d[k] = d[k][:, :, None, None]
